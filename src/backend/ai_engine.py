    ...
    def _get_openai_suggestions(self, code: str, language: str, errors: List = None) -> List[str]:
        ...
        try:
            ...
            payload = {
                'model': self.model,
                'messages': [
                    {'role': 'system', 'content': 'You are a helpful code review assistant. Provide concise, practical suggestions.'},
                    {'role': 'user', 'content': prompt}
                ],
                'temperature': self.config.get('temperature', 0.7),
                'max_tokens': self.config.get('maxTokens', 500)
            }
...
Be concise and practical. Focus on the most important issues.
